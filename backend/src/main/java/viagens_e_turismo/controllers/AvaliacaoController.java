...
public class AvaliacaoController {
    ...
    @GetMapping("/pacote/{pacoteId}/cliente/{clienteId}")
    public ResponseEntity<Avaliacao> findByPacoteAndCliente(@PathVariable int pacoteId, @PathVariable int clienteId){
        return ResponseEntity.status(HttpStatus.OK).body(avaliacaoService.findByPacoteAndCliente(pacoteId, clienteId));
    } 
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Avaliacao> update(@PathVariable @Positive int id, @RequestBody @Valid AvaliacaoRecordDto avaliacaoRecordDto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(avaliacaoService.update(id, avaliacaoRecordDto));
    }
